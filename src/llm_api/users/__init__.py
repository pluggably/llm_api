...
def _get_encryption_key() -> bytes:
    """Get or generate the encryption key for provider keys."""
    settings = get_settings()
    if settings.encryption_key:
        # Derive a proper Fernet key from the configured key
        key = hashlib.sha256(settings.encryption_key.encode()).digest()
        return Fernet.generate_key()  # Use proper Fernet key generation
    # Generate a key (should be persisted in production)
