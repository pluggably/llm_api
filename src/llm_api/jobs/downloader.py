...
class DownloadService:
        ...
        if source.type == "huggingface":
            ...
            asyncio.create_task(
                self._download_from_huggingface(
                    ...
                    model_id=model.id,
                    repo_id=repo_id,
                    filename=request.options.revision if request.options else None,
                )
            )
            return job
    ...
    async def _download_from_huggingface(
        ...
        try:
            ...
            # For GGUF models, find the appropriate quantization
            ...
            try:
                api = hf_hub.HfApi()
                file_info = api.hf_hub_url(repo_id, target_filename)
                # We can't easily get size without downloading, so we skip size check
                # or use repo_info for estimates
            except Exception:
